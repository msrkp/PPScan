var PAYLOADS = [
    ["XSS Prototype #1", "__proto__[e32a5ec9c99]", "ddcb362f1d60"],
    ["XSS Prototype #2", "__proto__.e32a5ec9c99", "ddcb362f1d60"],
    // ['XSS Prototype #3',  'x[__proto__][e32a5ec9c99]', 'ddcb362f1d60', ],
    // ['XSS Prototype #4',  'x.__proto__.e32a5ec9c99','ddcb362f1d60', ],
...
window.addEventListener("message", function(event) {
    if (event.data.url && event.data.foo == "baz") {
        if (window.name.search("ppdeadbeef1") == -1) {
            // avoiding recursion
        ...
        chrome.storage.sync.get("buster", function(data) {
            if (data.buster) {
                ...
                if (
                    url.search("dummy") == -1 &&
                    window.name.search("ppdeadbeef1") == -1
                ) {
                    // avooiding recursion
                ...
                if (window.name.search("ppdeadbeef1") == -1) {
                    ...
                    for (i = 0; i < PAYLOADS.length; i++) {
                        for (j = 0; j < 2; j++) {
                            ...
                            j
                                ?
                                locasdf.searchParams.append(PAYLOADS[i][1], PAYLOADS[i][2]) :
                                (locasdf.hash = "#" + PAYLOADS[i][1] + "=" + PAYLOADS[i][2]);
                            iframe.name = "ppdeadbeef1";
